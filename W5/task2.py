...
def matches(imageA, imageB, bboxA, bboxB):
    ...
    try:
        ...
        imageAc = cv2.cvtColor(imageA, cv2.COLOR_BGR2HSV)
        imageB = cv2.resize(imageB, (100, 100))
        imageBc = cv2.cvtColor(imageB, cv2.COLOR_BGR2HSV)
        ...
        h1 = cv2.calcHist([imageAc], [1], None, [256], [0, 256])
        h1 = cv2.normalize(h1, h1, 0, 1, cv2.NORM_MINMAX, -1)

        h2 = cv2.calcHist([imageBc], [1], None, [256], [0, 256])
        h2 = cv2.normalize(h2, h2, 0, 1, cv2.NORM_MINMAX, -1)

        colour = cv2.compareHist(h1, h2, 0)
...
def task2(save_frames=False, th = 1, mask = [0, 0],cam = ['c010', 'c011'], op = False, wz = 0, model='yolo3', seq = 'S03'):
    ...
    for frame in range(int(500), int(video_n_frames)):
        ...
        img2 = cv2.imread(path_to_frames2 + ('/frame_{:04d}.jpg'.format(frame + 1)))
            ...
            if matched_det1:
                if (matched_det1.bbox[3] > mask[0] \
                    ...
                    if track.buffer == 4:
                        ...
                        max_good = []
                        for t in track2_tot:
                            good, colour = matches(img1, t[2], matched_det1, t[1])
                            print(len(good)*abs(colour))
                            if len(good)*abs(colour) > len(max_good):
                                max_good = good
                                save_t = t
                        if len(max_good) > 25:
                            remove_id = track.id
            ...
            if matched_det2:
                if (matched_det2.bbox[3] > mask[2] \
                    ...
                    if track.buffer == 4:
                        ...
                        max_good = []
                        for t in track1_tot:
                            good, colour = matches(t[2], img2 , t[1], matched_det2)
                            print(len(good)*abs(colour))
                            if len(good)*abs(colour) > len(max_good):
                                max_good = good
                                save_t = t
