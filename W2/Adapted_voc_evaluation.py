...
def average_precision(gt_list, pred_list, confidence_score=True):
    ...
    if confidence_score :
        sorted_ind = np.argsort([det.frame for det in pred_list])
...
def voc_ap(gt_list, pred_list, ovthresh=0.5):
    ...
    class_recs = []
    npos = 0
    for R in gt_list.values():
        bbox = np.array([det.bbox for det in R])
        det = [False] * len(R)
        npos += len(R)
        class_recs.append({"bbox": bbox, "det": det})

    image_ids = [det.id for det in pred_list]
    BB = np.array([det.bbox for det in pred_list]).reshape(-1, 4)
