...
def remove_bg2(mu, sigma, alpha, frames_path, initial_frame, final_frame, animation=False, adaptive=True,
               rho=0.2, color_space=cv2.COLOR_BGR2GRAY, channels=(0)):
    c = 0
    detected_bb = []
    for i in tqdm(range(initial_frame, final_frame)):
        # read image
    ...
    # cv2.waitKey()
    if animation:
        frames_to_gif('bg_removal2_a{}_p{}_{}.gif'.format(alpha, rho, color_space), frames)
...
def fg_segmentation_to_boxes(frame, i, img, box_min_size=(10, 10), cls='car'):
    ...
    # cv2.waitKey()
    contours, _ = cv2.findContours(framee, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
